...
(function (root, factory) {
  ...
  function generateList({ items, ulClass = "", liClass = "", ulId = "" }) {
    const listItems = items
      .map((item) =>
        createElement("li", { class: liClass, id: item.id }, item.content)
      )
      .join("");
    ...
    const rows = data
      .map((row) =>
        createElement(
          "tr",
          {},
          row.map((cell) => createElement("td", {}, cell)).join("")
        )
      )
      .join("");
  ...
  function generateLinks({ lstLinks, linkClass = "", containerId = "" }) {
    ...
    return createElement(
      "div",
      { class: linkClass, id: containerId },
      linkItems
    );
  }
  ...
  function generateHref({ url, text, linkClass = "", linkId = "" }) {
    return createElement(
      "a",
      { href: url, class: linkClass, id: linkId },
      text
    );
  }
